...
export default { 
    ...
    houses_get: (req, res) => { 
        ...
        if(!house) { res.sendStatus(404); return; }

        res.status(200).json( 
           (({ id, name } ) => ({ id, name }))(house)
        ); 
    },
    houses_get_locks: (req, res) => { 
        ...
        res.status(200).json( 
            locks.map( ({ id, category, name, status }) => ({ id, category, name, status }))
        ) 
    },
    houses_get_lock: (req, res) => { 
        ...
        if(!lock) { res.sendStatus(404); return; }
        
        res.status(200).json( 
            ( ({ id, category, name, status }) => ({ id, category, name, status }))(lock)
         ); 
    },
    houses_update_lock: async (req, res) => { 
        const houseId = parseInt(req.params.houseId);
        const lockId = parseInt(req.params.houseId);
        const house = House.get(houseId);
        ...
        if(lock.isOffline()) { res.sendStatus(503); return; }
        
        if(!Lock.validStatusValues.includes(req.body.status)){ res.sendStatus(400); return; }
